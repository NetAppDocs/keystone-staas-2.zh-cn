...
** *Keystone 版本*：用于订阅的 Keystone 服务版本。性能服务级别的费率计划规则可能因订阅版本而异。要了解有关版本 v1 的更多信息，请参阅 https://docs.netapp.com/us-en/keystone/index.html["Keystone订阅服务 | 版本 1 文档"^]。默认情况下，此列是隐藏的，可以使用列选择器image:column-selector.png["列选择器图标"]图标启用。
** *使用类型*：您可能已经订阅了多个 Keystone 版本。性能服务级别的费率计划规则可能因订阅版本而异。通过查看此列中的值，您可以知道使用类型是按预配、物理还是逻辑使用计费。要了解有关版本 1 的更多信息，请参阅 https://docs.netapp.com/us-en/keystone/index.html["Keystone订阅服务 | 版本 1 文档"^]。
** *计费周期*：订阅的发票周期。
** *性能服务级别*：与订阅相关的费率计划数量，每个费率计划对应特定的性能服务级别和承诺容量。
** *最高容量*：订阅内消耗的最大容量。
** *订阅到期天数*：在到期日期旁边显示一个图标，表明订阅即将到期以及是否会自动续订。此状态为只读，反映了您的合同条款。要查看合同详细信息，请参阅 link:../integrations/subscription-timeline.html["查看Keystone订阅的时间线"]。
+
...
TIP: 如果您已订阅高级数据保护附加服务，则可以单击订阅号下的工具提示，以在 MetroCluster 设置中查看合作伙伴订阅的跟踪 ID。要查看 MetroCluster 配置中合作伙伴订阅的详细使用情况，请参阅link:../integrations/adp-tab.html["查看 MetroCluster 订阅消费和运行状况"]。
